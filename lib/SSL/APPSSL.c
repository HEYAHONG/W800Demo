...
#define malloc(x) tls_mem_alloc(x)
#define free(x)  tls_mem_free(x)
...
// TLS operations
#define mbedtls_printf(fmt,args...) wm_printf("[ssl]"fmt ,##args)
...
{
    //调试mbedtls栈时取消下列行的注释
	wm_printf("[ssl_debug]:ret=%d,func=%s,line=%d,str=%s",ret,file,line,str);
}
                     ...
                     unsigned int ca_crt_len, unsigned int timeout_ms)
{
    ...
    fcntl(wxpp_handle->client_fd.fd,F_SETFL,O_NONBLOCK);
    fcntl(wxpp_handle->client_fd.fd,F_SETFL,O_NDELAY);
